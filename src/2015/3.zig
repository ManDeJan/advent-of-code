...
pub fn run(input: aoc.Input) !aoc.Output {
    ...
    try santa_map.put(@bitCast(santa_pos), {});
    ...
    try santa_and_robot_map.put(@bitCast(santa2_pos), {});

    for (input, 0..) |dir, i| {
        santa_pos.move(dir);
        const entry = try santa_map.getOrPut(@bitCast(santa_pos));
        if (!entry.found_existing) part1 += 1;

        var santa_or_robot = &(if (i % 2 == 0) santa2_pos else santaR_pos);
        santa_or_robot.move(dir);
        const entry2 = try santa_and_robot_map.getOrPut(@bitCast(santa_or_robot.*));
