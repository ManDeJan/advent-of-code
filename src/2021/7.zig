...
pub noinline fn run(input: aoc.Input) !aoc.Output {
    ...
    std.mem.sort(u16, nums.items, {}, comptime std.sort.asc(u16));

    const len: u32 = @intCast(nums.items.len);
    const mean1 = sum / len;
    const mean2 = mean1 + 1;
    const median = if (nums.items[len / 2] % 2 == 0) (nums.items[len / 2 - 1] + nums.items[len / 2]) / 2 else nums.items[len / 2];
    var part2_1: u32 = 0;
    var part2_2: u32 = 0;
    for (nums.items) |num| {
