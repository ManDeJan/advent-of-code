...
pub fn run(input: aoc.Input) anyerror!aoc.Output {
    ...
    while (lines.next()) |line| {
        ...
        rules[perfectHashBag(color, first_start_col)] = new_rule;
    }

    const shiny_gold_rule = &rules[perfectHashBag("shiny gold", 6)];
    part2 = countBagsRecursive(shiny_gold_rule) - 1; // minus one because we count the bag itself
    shiny_gold_rule.has_gold_bag = true;

    for (&rules) |*rule| {
        part1 += @as(i64, @intFromBool(containsShinyGoldBag(rule)));
    }

    return aoc.Output{ .part1 = part1, .part2 = part2 };
}

fn countBagsRecursive(root_rule: *Rule) i64 {
    var total: i64 = 1;
    for (&root_rule.contains, 0..) |*child_rule, i| {
        if (child_rule.* == null) break;
        total += @as(i64, root_rule.contains_amt[i]) *
            countBagsRecursive(child_rule.*.?);
    }
    return total;
}

fn containsShinyGoldBag(root_rule: *Rule) bool {
    if (root_rule.has_gold_bag) |value| return value;
    root_rule.has_gold_bag = false;
    for (&root_rule.contains) |*child_rule| {
        if (child_rule.* == null) break;
        root_rule.has_gold_bag.? = containsShinyGoldBag(child_rule.*.?);
...
const lookup_table = blk: {
    ...
    for (bag_names) |bag| {
        ...
        _lookup_table[hashBagImpl(bag, start_col)] = i;
        i += 1;
    }
...
fn hashBagImpl(bag: []const u8, start_col: usize) u16 {
    const key = @as(usize, bag[0]) << 0 ^
        @as(usize, bag[1]) << 3 ^
        @as(usize, bag[0 + start_col]) << 10 ^
        @as(usize, bag[1 + start_col]) << 10 ^
        @as(usize, bag.len) << 7;
    return @intCast((key ^ 2985) % 4739);
}

fn perfectHashBag(bag: []const u8, start_col: usize) u16 {
    return lookup_table[hashBagImpl(bag, start_col)];
}
